...
wrap(HierarchyWidget, 'render', function (render) {
    ...
    if (!this.$('.h-delete-checked-btn').length && accessLevel !== undefined && accessLevel >= AccessType.WRITE) {
        ...
        if (self.folderListView) {
            self.listenTo(self.folderListView, 'g:checkboxesChanged', updateDeleteBtnVisibility);
        }
        if (self.itemListView) {
            self.listenTo(self.itemListView, 'g:checkboxesChanged', updateDeleteBtnVisibility);
        }
    }
